...
function Base.getindex(::NullInvariants, i...)
    error("Invariants were indexed but the invariants are `NullInvariants`. You likely forgot to pass invariants to the Equation!")
end
function Base.iterate(::NullInvariants)
    error("Invariants were indexed but the invariants are `NullInvariants`. You likely forgot to pass invariants to the Equation!")
end
...
function Base.getindex(::NullParameters, i...)
    error("Parameters were indexed but the parameters are `NullParameters`. You likely forgot to pass parameters to the Equation!")
end
...
function Base.getindex(::NullPeriodicity, i...)
    error("Periodicity was indexed but the periodicity is `NullPeriodicity`. You likely forgot to pass periodicity to the Equation!")
end
function Base.iterate(::NullPeriodicity)
    error("Periodicity was indexed but the periodicity is `NullPeriodicity`. You likely forgot to pass periodicity to the Equation!")
end

const OptionalArray{arrayType} = Union{
    Nothing, arrayType} where {arrayType <: AbstractArray}
...
const OptionalPeriodicity = Union{
    Tuple{AT, AT}, NullPeriodicity} where {AT <: AbstractArray}
